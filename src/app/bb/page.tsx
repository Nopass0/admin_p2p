...
const BybitCabinetModal = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
  ...
  const save = () => {
    ...
    if (!bybitEmail || !apiKey || !apiSecret) return toast.error("Заполните все поля.");
    if (editing) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} size="2xl">
      <ModalContent className="max-h-[90dvh] overflow-y-auto">
        <ModalHeader>{editing ? "Редактировать кабинет" : isAdding ? "Добавить кабинет" : "Кабинеты Bybit"}</ModalHeader>
        <ModalBody className="space-y-4">
                  ...
                  <TableBody items={cabinets}>
                    {(item: any) => (
                      <TableRow key={item.id}>
                        <TableCell className="truncate max-w-[14ch]">{item.bybitEmail}</TableCell>
                        <TableCell>{item.bybitApiToken?.slice(0, 5)}…</TableCell>
          ...
          {(isAdding || editing) && (
            <div className="grid gap-4 md:grid-cols-2">
              <Input label="Bybit Email" name="bybitEmail" value={form.bybitEmail} onChange={onChange} isRequired />
              <Input label="API Key" name="apiKey" type="password" value={form.apiKey} onChange={onChange} isRequired />
              <Input label="API Secret" name="apiSecret" type="password" value={form.apiSecret} onChange={onChange} isRequired />
            </div>
          )}
        </ModalBody>
        <ModalFooter className="flex gap-2 justify-end flex-wrap">
          {(isAdding || editing) ? (
            <>
              ...
              <Button color="primary" isLoading={createMutation.isLoading || updateMutation.isLoading} onPress={save}>{editing ? "Сохранить" : "Добавить"}</Button>
            </>
  ...
  return (
    <div className="px-4 py-6 mx-auto w-full max-w-7xl">
      ...
      <div className="flex flex-wrap gap-2 justify-end mb-6">
        <Button color="secondary" variant="bordered" startContent={<Settings size={18} />} onPress={openModal}>Кабинеты Bybit</Button>
        <Button color="primary" startContent={<PlusIcon size={18} />} onPress={() => router.push("/bb/report/new")}>Создать отчёт</Button>
...
const Header = ({ report }: { report: any }) => (
  <div className="p-4 bg-gray-50 dark:bg-zinc-900 flex justify-between items-start gap-2 flex-wrap">
    <div className="flex items-center gap-2 min-w-0">
      ...
      <h3 className="truncate text-lg font-semibold max-w-[14ch] sm:max-w-none">{report.name}</h3>
    </div>
...
const Main = ({ report, idex, bybit }: { report: any; idex: any[]; bybit: any[] }) => (
...
const Footer = ({ id, onDelete }: { id: number; onDelete: (id: number) => void }) => (
  <div className="p-3 bg-gray-50 dark:bg-zinc-900 flex items-center gap-2">
    <Button className="flex-grow" color="primary" startContent={<Eye size={18} />} onPress={() => location.assign(`/bb/report/${id}`)}>Просмотр</Button>
...
function CabinetsBlock({ idex, bybit }: { idex: any[]; bybit: any[] }) {
  return (
    <div className="space-y-2">
      <CabinetSection title="IDEX кабинеты" list={idex.map((c) => ({ label: c.idexId ?? c.id, start: c.startDate ?? c.start, end: c.endDate ?? c.end }))} badge="blue" />
      <CabinetSection title="Bybit кабинеты" list={bybit.map((c) => ({ label: c.email ?? c.id, start: c.startDate ?? c.start, end: c.endDate ?? c.end }))} badge="yellow" />
    </div>
  );
}
...
function CabinetSection({ title, list, badge }: { title: string; list: { label: string; start: string; end: string }[]; badge: "blue" | "yellow" }) {
  const cls = badge === "blue" ? "bg-blue-100 dark:bg-blue-900/40 text-blue-800 dark:text-blue-300" : "bg-yellow-100 dark:bg-yellow-900/40 text-yellow-800 dark:text-yellow-300";
  return (
    <div>
      <Label>{title}</Label>
