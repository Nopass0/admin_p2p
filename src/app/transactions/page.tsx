...
// Создаем клиентский компонент для работы с параметрами URL
function SearchParamsManager({ onParamsLoaded }: { onParamsLoaded: (userId: number | undefined) => void }) {
  const searchParams = useSearchParams();
  
  useEffect(() => {
    const userId = searchParams.get("userId") ? Number(searchParams.get("userId")) : undefined;
    onParamsLoaded(userId);
...
export default function TransactionsPage() {
  ...
  // Обработчик для получения userId из URL-параметров
  const handleParamsLoaded = (loadedUserId: number | undefined) => {
  ...
  const {
    ...
    enabled: !!userId && !isNaN(Number(userId)) && isParamsLoaded
  });
  ...
  useEffect(() => {
    if (userId && isParamsLoaded) {
      void refetch();
      void refetchStats();
    }
  }, [userId, isParamsLoaded, refetch, refetchStats]);
  ...
  return (
    <div className="p-6">
      {/* Обертываем компонент с useSearchParams в Suspense */}
      <Suspense fallback={<div>Загрузка...</div>}>
        <SearchParamsManager onParamsLoaded={handleParamsLoaded} />
      </Suspense>
