...
## Последние обновления (21.04.2025)

### Исправленные API методы в src/server/api/routers/bb.ts

- `getIdexTransactionsForReport` - метод для получения IDEX транзакций для отчета
  - Исправлены ошибки с типами данных и несуществующими полями
  - Добавлена обработка ошибок при парсинге JSON-полей

- `getBybitTransactionsForReport` - метод для получения Bybit транзакций для отчета
  - Исправлено несуществующее поле `transactionTime` на `dateTime`

- `getMatchBybitReportById` - метод для получения отчета по ID
  - Исправлено поле `matches` на `bybitClipMatches`
...
│   │   ├── bb/                 # Функциональность сопоставления Bybit
│   │   │   ├── page.tsx        # Основная страница: список отчетов Bybit, управление кабинетами Bybit
│   │   │   └── report/
│   │   │       ├── [id]/
│   │   │       │   └── page.tsx # Страница детализации и сопоставления отчета Bybit
│   │   │       └── new/
│   │   │           └── page.tsx # Страница создания нового отчета Bybit
...
│   │   │   │   └── bb.ts       # tRPC роутер для операций с Bybit (кабинеты, отчеты, транзакции, сопоставление)
...
### Модуль сопоставления Bybit и IDEX транзакций (`/bb`)
- Создание и управление отчетами по сопоставлению транзакций между платформами Bybit и IDEX
- Управление кабинетами Bybit (создание, редактирование, удаление)
- Создание отчетов с настраиваемыми периодами для каждого кабинета (Bybit и IDEX)
- Автоматический мэтчинг транзакций при создании отчета с учетом временного окна (30 минут) между транзакциями
- Возможность ручного добавления и удаления сопоставлений в отчете
- Расчет и отображение статистики по отчету (количество совпадений, общая прибыль, средняя прибыль, процент успешности)
- Выбор пользователя для отчета с фильтрацией доступных кабинетов
- Реализованы индивидуальные настройки периодов для каждого кабинета в отчете
